...
import { WALMART_IMPACT_ANALYSIS_SB_ADVERTISING_BASE_URL } from 'src/constants';
import {
  IAnalysisFilter,
  IAnalysisTableData,
...
export const WalmartAnalysisSBService = {
  getImpactAnalysis: (
    ...
    return axiosInstance.post<IAPIResponse<TImpactAnalysisDataList>>(
      `${WALMART_IMPACT_ANALYSIS_SB_ADVERTISING_BASE_URL}/graph`,
      body,
      {
        signal,
      }
  ...
  getImpactAnalysisTable: (
    ...
    >(
      `${WALMART_IMPACT_ANALYSIS_SB_ADVERTISING_BASE_URL}/table?page=${page}&pageSize=${pageSize}`,
      bodyData
    );
  },
};
