...
import { WALMART_IMPACT_ANALYSIS_OVERALL_ADVERTISING_BASE_URL } from 'src/constants';
import {
  IAnalysisFilter,
  IAnalysisTableData,
...
export const WalmartAnalysisOverallService = {
  getImpactAnalysis: (
    ...
    return axiosInstance.post<IAPIResponse<TImpactAnalysisDataList>>(
      `${WALMART_IMPACT_ANALYSIS_OVERALL_ADVERTISING_BASE_URL}/graph`,
      body,
      {
        signal,
      }
  ...
  getImpactAnalysisTable: (
    ...
    >(
      `${WALMART_IMPACT_ANALYSIS_OVERALL_ADVERTISING_BASE_URL}/table?page=${page}&pageSize=${pageSize}`,
      bodyData,
      { signal }
    );
  },
