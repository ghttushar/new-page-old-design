...
import { AMAZON_IMPACT_ANALYSIS_SD_ADVERTISING_BASE_URL } from 'src/constants';
import {
  IAnalysisFilter,
  IAnalysisTableData,
...
export const AnalysisSDService = {
  getImpactAnalysis: (
    ...
    return axiosInstance.post<IAPIResponse<TImpactAnalysisDataList>>(
      `${AMAZON_IMPACT_ANALYSIS_SD_ADVERTISING_BASE_URL}/graph`,
      body,
      {
        signal,
      }
  ...
  getImpactAnalysisTable: (
    ...
    >(
      `${AMAZON_IMPACT_ANALYSIS_SD_ADVERTISING_BASE_URL}/table?page=${page}&pageSize=${pageSize}`,
      bodyData,
      { signal }
    );
  },
