...
export default function TotalBudgetAction({
  setTableData,
}: IBudgetActionProps) {
  ...
  const handleBudgetClick = () => {
    dispatch(setIsOpenTotalBudgetDialog());
  };
  ...
  const handleBudgetApply = () => {
    ...
    editState.forEach((row) => {
      if (selectedRowIds.includes(row.id as string | number)) {
        if (row.budgetType === WalmartBudgetTypeEnum.DAILY) return;
        ...
        if (
          (walmartAccount?.accountType === WalmartAccountTypeEnum.THIRD_PARTY &&
            updatedBudget < WALMART_3P_TOTAL_BUDGET_MIN) ||
          (walmartAccount?.accountType === WalmartAccountTypeEnum.FIRST_PARTY &&
            updatedBudget < WALMART_1P_TOTAL_BUDGET_MIN) ||
          ...
          if (
            ...
            setErrorMsg(
              `Some total budgets are lower than ${displayValue(
                WALMART_3P_TOTAL_BUDGET_MIN,
                false
          ...
          if (updatedBudget > WALMART_BUDGET_MAX) {
            setErrorMsg(
              `Total budget cannot exceed ${displayValue(
                WALMART_BUDGET_MAX,
                false
              )} .`
            );
    ...
    const updatedState = editState.map((row) => {
      if (
        selectedRowIds.includes(row.id as string) &&
        row.budgetType !== WalmartBudgetTypeEnum.DAILY
      ) {
        return {
          ...row,
          totalBudget: getCalculatedBudgetBid(
            Number(row.totalBudget),
            newBudget,
            selectedAdjustment.value
          ),
