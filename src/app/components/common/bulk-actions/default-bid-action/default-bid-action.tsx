...
type IDefaultBidActionProps = IEditBulkActionProp;

export default function DefaultBidAction({
  setTableData,
}: IDefaultBidActionProps) {
  const [selectedAdjustment, setSelectedAdjustment] = useState<
    IDropdownItem<Adjustments>
  >(adjustmentOptions[0]);
  const [newDefaultBid, setNewDefaultBid] = useState<number | typeof NaN>(0);
  ...
  const handleDefaultBidApply = () => {
    ...
    editState.forEach((row: IAdGroup | ISDAdGroup) => {
      if (selectedRowIds.includes(row.id as string | number)) {
        ...
        if (minLimitErrMsg) {
          _unwantedDefaultBids.push(updatedDefaultBid);
          setErrorMsg(
            `Some default bids are lower than 
             ${displayValue(
        ...
        if (maxLimitErrMsg) {
          _unwantedDefaultBids.push(updatedDefaultBid);
          setErrorMsg(
            `Some max bids are exceeding ${displayValue(
    ...
    if (_unwantedDefaultBids.length) {
    ...
    const updatedState = editState.map((row) => {
      if (selectedRowIds.includes(row.id as string | number)) {
        return {
          ...row,
          defaultBid: getCalculatedBudgetBid(
            row.defaultBid,
            newDefaultBid,
            selectedAdjustment.value
          ),
  ...
  const handleDefaultBidChange = (value: number) => {
    setNewDefaultBid(value);
  };
  ...
  return (
    <div className={styles.buttonDialogContainer}>
      <TextButton
        label="Default Bid"
        handleClick={handleDefaultBidClick}
        isVisible={true}
      ...
      {isOpenBidDialog === true && (
        <ActionConfirmationDialog
          dialogMessage="Are you sure you adjust default bid? This action might affect the spends."
          onApply={handleDefaultBidApply}
          onClose={() => dispatch(resetDialogs())}
          isApplyDisabled={
            selectedRowIds.length === 0 ||
            newDefaultBid === null ||
            newDefaultBid === undefined
          }
        ...
        >
          <DropdownTextfield
            ...
            fieldValue={newDefaultBid}
            onValueChange={handleDefaultBidChange}
