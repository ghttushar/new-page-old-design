...
export default function AdvertisingCampLevelAutoTargeting<
...
>({
    ...
    options: {
      onSuccess: (response) => {
        queryClient.invalidateQueries({
          queryKey: [QueryKeyEnums.AMZ_SP_CAMPAIGN_LVL_AUTO_FETCH],
        });
  ...
  useEffect(() => {
    ...
    dispatch(
      setSelectedAdvertisingNavTitle(SpCampaignLevelTitles.AUTO_TARGETING)
    );
  }, [dispatch, updatedPerformanceOptions]);

  const getCampLevelAutoTargetingDownload = useCallback(
  ...
  const fetchAutoTargeting = useAppQuery({
    queryKey: [
      QueryKeyEnums.AMZ_SP_CAMPAIGN_LVL_AUTO_FETCH,
      {
        appliedFilters,
    ...
    queryFn: () =>
      spAdvertisingServices.getAutoTargeting(
        ...
        paginationModel.pageIndex + 1,
        paginationModel.pageSize,
        getFormattedSortModel(SpCampaignLevelTitles.AUTO_TARGETING, sortModel),
        searchText
      ),
    enabled:
      selectedAdvertisingNavTitle === SpCampaignLevelTitles.AUTO_TARGETING,
  });
  ...
  const handleDownload = useCallback(
    async (isAllDownload: boolean) => {
      ...
      const data: Record<string, unknown>[] =
        (await getCampLevelAutoTargetingDownload(
          true,
          isAllDownload
    ...
    [dispatch, getCampLevelAutoTargetingDownload]
  );
  ...
  return (
    <AdvertisingRenderingComponents
      ...
      exportFileTitle={genExportFileName('amazon-sp-camp', 'Auto Targeting')}
