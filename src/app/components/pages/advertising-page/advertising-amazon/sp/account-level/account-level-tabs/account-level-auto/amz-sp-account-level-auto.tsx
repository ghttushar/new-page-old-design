...
export default function AdvertisingAccountLevelAuto() {
  ...
  const setSelectedColumnsHandler = (
    ...
    columnFilterUtils.syncStoredColumnFilters(
      SpAccountLevelTitles.AUTO_TARGETING,
      selectedColumns
    );
    ...
    options: {
      onSuccess: (response) => {
        queryClient.invalidateQueries({
          queryKey: [QueryKeyEnums.AMZ_SP_ACCOUNT_LVL_AUTO_FETCH],
        });
  ...
  useEffect(() => {
    ...
    dispatch(
      setSelectedAdvertisingNavTitle(SpAccountLevelTitles.AUTO_TARGETING)
    );
  }, [dispatch]);

  const getAutoTargetingDownload = useCallback(
    async (isDownload = false, isAllDownload = false) => {
      const res = spAdvertisingServices
        .getAutoTargeting(
          ...
          getFormattedSortModel(SpAccountLevelTitles.AUTO_TARGETING, sortModel),
          searchText
        )
        .then((res) => {
          ...
          dispatch(
            showSuccessToastMessage({
              ...
              description: 'Auto Targeting downloaded successfully.',
            })
          );
          return data;
        });
  ...
  const fetchAutoTargeting = useAppQuery({
    queryKey: [
      QueryKeyEnums.AMZ_SP_ACCOUNT_LVL_AUTO_FETCH,
      {
        appliedFilters,
    ...
    queryFn: () =>
      spAdvertisingServices.getAutoTargeting(
        appliedFilters,
        getAmazonAdvertisingFilters(
          appliedAdvertisingFilters,
          appliedAdvertisingFilters.customDateRange,
    ...
    enabled:
      selectedAdvertisingNavTitle === SpAccountLevelTitles.AUTO_TARGETING,
  });
  ...
  const fetchPerformanceGraph = useAppQuery({
    ...
    queryFn: () =>
      spAdvertisingServices.getPerformanceGraph(
        ...
        searchText,
        AmazonAdvertisingTableTypesEnum.AUTO_TARGETING
      ),
    enabled:
      selectedAdvertisingNavTitle === SpAccountLevelTitles.AUTO_TARGETING,
  });
  ...
  useEffect(() => {
    ...
    if (fetchAutoTargeting.data) {
      let data = fetchAutoTargeting.data.data.data.data;
      data = data.map((row) => {
        const id = `${row.targetId}`;
        return {
          id,
      ...
      const totalRows = fetchAutoTargeting.data.data.data.pagination.totalItems;
      setTotalRowCount(parseInt(totalRows as string));
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [fetchAutoTargeting.data, dispatch]);
  ...
  const handleDownload = useCallback(
    async (isAllDownload: boolean) => {
      ...
      const data: Record<string, unknown>[] = (await getAutoTargetingDownload(
        true,
        isAllDownload
    ...
    [dispatch, getAutoTargetingDownload]
  );
  ...
  return (
    <AdvertisingRenderingComponents
      ...
      isTableLoading={
        fetchAutoTargeting.isLoading || fetchAutoTargeting.isRefetching
      }
      initialColumns={initialColumns}
      selectedColumns={selectedColumns}
      handleSelectedColumns={setSelectedColumnsHandler}
      exportFileTitle={genExportFileName('amazon-sp', 'Auto_Targeting')}
