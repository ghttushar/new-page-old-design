...
export default function AdvertisingAdgrpLevelNegKT<
...
>({
  ...
  useEffect(() => {
    ...
    dispatch(
      setSelectedAdvertisingNavTitle(SpAdGroupLevelTitles.NEG_TARGETING_KEYWORD)
    );
  }, [dispatch, updatedPerformanceOptions]);

  const getAdGroupNegativeKeywordTargetingDownload = useCallback(
  ...
  const fetchNegKeywordTargeting = useAppQuery({
    queryKey: [
      QueryKeyEnums.AMZ_SP_ADGROUP_LVL_NEG_KT_FETCH,
      {
        appliedFilters,
  ...
  const handleDownload = useCallback(
    async (isAllDownload: boolean) => {
      ...
      const data: Record<string, unknown>[] =
        (await getAdGroupNegativeKeywordTargetingDownload(
          true,
          isAllDownload
    ...
    [dispatch, getAdGroupNegativeKeywordTargetingDownload]
  );
