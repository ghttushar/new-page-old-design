...
export default function AdvertisingAdgrpLevelKT<
...
>({
  ...
  useEffect(() => {
    ...
    dispatch(
      setSelectedAdvertisingNavTitle(SpAdGroupLevelTitles.KEYWORD_TARGETING)
    );
  }, [dispatch, updatedPerformanceOptions]);

  const getAdGroupKeywordTargetingDownload = useCallback(
        ...
        .then((res) => {
          ...
          dispatch(
            showSuccessToastMessage({
              ...
              description: 'Keyword Targets downloaded successfully.',
            })
          );
          return data;
        });
  ...
  const fetchKeywordTargeting = useAppQuery({
    queryKey: [
      QueryKeyEnums.AMZ_SP_ADGROUP_LVL_KT_FETCH,
      {
        appliedFilters,
    ...
    enabled:
      selectedAdvertisingNavTitle === SpAdGroupLevelTitles.KEYWORD_TARGETING,
  });
  ...
  const handleDownload = useCallback(
    async (isAllDownload: boolean) => {
      ...
      const data: Record<string, unknown>[] =
        (await getAdGroupKeywordTargetingDownload(
          true,
          isAllDownload
        )) as unknown as Record<string, unknown>[];
      return data;
    },
    [dispatch, getAdGroupKeywordTargetingDownload]
  );
