...
export default function AdvertisingAdgrpLevelSearchTerm<
...
>({
  ...
  const setSelectedColumnsHandler = (
    ...
    columnFilterUtils.syncStoredColumnFilters(
      SpAdGroupLevelTitles.SEARCH_TERM,
      selectedColumns
    );
  ...
  useEffect(() => {
    ...
    dispatch(setSelectedAdvertisingNavTitle(SpAdGroupLevelTitles.SEARCH_TERM));
  }, [dispatch, updatedPerformanceOptions]);

  const getAdGroupSearchTermDownload = useCallback(
    async (isDownload = false, isAllDownload = false) => {
      const res = spAdvertisingServices
        .getSearchTerms(
          ...
          getFormattedSortModel(SpAdGroupLevelTitles.SEARCH_TERM, sortModel),
          searchText
        )
        .then((res) => {
  ...
  const fetchSearchTerm = useAppQuery({
    queryKey: [
      QueryKeyEnums.AMZ_SP_ADGROUP_LVL_SEARCH_TERM_FETCH,
      {
        appliedFilters,
    ...
    enabled: selectedAdvertisingNavTitle === SpAdGroupLevelTitles.SEARCH_TERM,
  });
  ...
  const handleDownload = useCallback(
    async (isAllDownload: boolean) => {
      ...
      const data: Record<string, unknown>[] =
        (await getAdGroupSearchTermDownload(
          true,
          isAllDownload
    ...
    [dispatch, getAdGroupSearchTermDownload]
  );
