...
import {
  IEditAccessAdGroup,
  IEditAccessSBAdGroupUpdateBody,
...
export default function AdvertisingSBCampLevelAdGroup<
...
>({
  ...
  const setSelectedColumnsHandler = (
    ...
    columnFilterUtils.syncStoredColumnFilters(
      SbCampaignLevelTitles.AD_GROUP,
      selectedColumns
    );
  ...
  const { mutateAsync: editAccessMutateAll } = useAppMutation({
    mutationFn: async (body: IEditAccessSBAdGroupUpdateBody) => {
      return await EditAccessSBServices.updateSBAdGroup(body);
    },
    options: {
      onSettled: () => {
      ...
      onSuccess: (response) => {
        queryClient.invalidateQueries({
          queryKey: [QueryKeyEnums.AMZ_SB_CAMPAIGN_LVL_ADGROUP_FETCH],
        });
  ...
  const confirmEditSaveClick = async () => {
    ...
    const updatedAdGroupsBody: IEditAccessSBAdGroupUpdateBody = {
    ...
    async (isDownload = false, isAllDownload = false) => {
      const res = sbAdvertisingCampaignLevelServices
        .getSBAdGroups(
          ...
          getFormattedSortModel(SbCampaignLevelTitles.AD_GROUP, sortModel),
          searchText
        )
        .then((res) => {
  ...
  const fetchAdGroups = useAppQuery({
    queryKey: [
      QueryKeyEnums.AMZ_SB_CAMPAIGN_LVL_ADGROUP_FETCH,
      {
        appliedFilters,
    ...
    queryFn: () =>
      sbAdvertisingCampaignLevelServices.getSBAdGroups(
        appliedFilters,
        advertisingFiltersWithNoDownload,
        paginationModel.pageIndex + 1,
        paginationModel.pageSize,
    ...
    enabled: selectedAdvertisingNavTitle === SbCampaignLevelTitles.AD_GROUP,
  });
  ...
  useEffect(() => {
    ...
    dispatch(setSelectedAdvertisingNavTitle(SbCampaignLevelTitles.AD_GROUP));
  ...
  return (
    <AdvertisingRenderingComponents
      ...
      exportFileTitle={genExportFileName('amazon-sb', 'Ad Groups')}
