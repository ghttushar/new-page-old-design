...
export default function AdvertisingOverallAccountLevelProductAds() {
  ...
  const setSelectedColumnsHandler = (
    ...
    columnFilterUtils.syncStoredColumnFilters(
      OverallAccountLevelTitles.PRODUCT_ADS,
      selectedColumns
    );
  ...
  useEffect(() => {
    ...
    dispatch(
      setSelectedAdvertisingNavTitle(OverallAccountLevelTitles.PRODUCT_ADS)
    );
  }, [dispatch]);

  const getProductAdsDownload = useCallback(
  ...
  const fetchProductAds = useAppQuery({
    queryKey: [
      QueryKeyEnums.AMZ_OVERALL_ACCOUNT_LVL_PRODUCT_ADS_FETCH,
      {
        appliedFilters,
    ...
    queryFn: () =>
      overallAdvertisingAccountLevelServices.getProductAdsData(
        appliedFilters,
        getAmazonAdvertisingFilters(
          appliedAdvertisingFilters,
          appliedAdvertisingFilters.customDateRange,
        ...
        paginationModel.pageIndex + 1,
        paginationModel.pageSize,
        getFormattedSortModel(OverallAccountLevelTitles.PRODUCT_ADS, sortModel),
        searchText
      ),
    enabled:
      selectedAdvertisingNavTitle === OverallAccountLevelTitles.PRODUCT_ADS,
  });
  ...
  const handleDownload = useCallback(
    async (isAllDownload: boolean) => {
      ...
      const data: Record<string, unknown>[] = (await getProductAdsDownload(
        true,
        isAllDownload
    ...
    [dispatch, getProductAdsDownload]
  );
  ...
  return (
    <AdvertisingRenderingComponents
      ...
      exportFileTitle={genExportFileName('amazon-overall', 'Product Ads')}
