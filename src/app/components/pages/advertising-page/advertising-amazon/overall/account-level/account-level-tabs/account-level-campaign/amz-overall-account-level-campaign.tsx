...
export default function AdvertisingOverallAccountLevelCampaign() {
  ...
  const setSelectedColumnsHandler = (
    ...
    columnFilterUtils.syncStoredColumnFilters(
      OverallAccountLevelTitles.CAMPAIGNS,
      selectedColumns
    );
  ...
  useEffect(() => {
    ...
    dispatch(
      setSelectedAdvertisingNavTitle(OverallAccountLevelTitles.CAMPAIGNS)
    );
  }, [dispatch]);

  const getCampaignDownload = useCallback(
    async (isDownload = false, isAllDownload = false) => {
      const res = overallAdvertisingAccountLevelServices
        .getCampaignData(
          ...
          getFormattedSortModel(OverallAccountLevelTitles.CAMPAIGNS, sortModel),
          searchText
        )
        .then((res) => {
  ...
  const fetchCampaigns = useAppQuery({
    queryKey: [
      QueryKeyEnums.AMZ_OVERALL_ACCOUNT_LVL_CAMPAIGN_FETCH,
      {
        appliedFilters,
    ...
    queryFn: ({ signal }) =>
      overallAdvertisingAccountLevelServices.getCampaignData(
        appliedFilters,
        getAmazonAdvertisingFilters(
          appliedAdvertisingFilters,
          appliedAdvertisingFilters.customDateRange,
    ...
    enabled:
      selectedAdvertisingNavTitle === OverallAccountLevelTitles.CAMPAIGNS,
  });
  ...
  const handleDownload = useCallback(
    async (isAllDownload: boolean) => {
      ...
      const data: Record<string, unknown>[] = (await getCampaignDownload(
        true,
        isAllDownload
  ...
  return (
    <AdvertisingRenderingComponents
      ...
      exportFileTitle={genExportFileName('amazon-overall', 'Campaigns')}
