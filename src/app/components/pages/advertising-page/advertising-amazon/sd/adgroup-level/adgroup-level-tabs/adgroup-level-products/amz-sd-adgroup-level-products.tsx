...
export default function AdvertisingSDAdGroupLevelProductAds<
...
>({
  ...
  useEffect(() => {
    ...
    dispatch(setSelectedAdvertisingNavTitle(SdAdGroupLevelTitles.PRODUCT_ADS));
    ...
    (isDownload = false, isAllDownload = false) => {
      const res = sdAdvertisingServicesAdGroupLevel
        .getSDProductAdsData(
          ...
          getFormattedSortModel(SdAdGroupLevelTitles.PRODUCT_ADS, sortModel),
          searchText
        )
        .then((res) => {
  ...
  const fetchProductAds = useAppQuery({
    queryKey: [
      QueryKeyEnums.AMZ_SD_ADGROUP_LVL_PRODUCT_ADS_FETCH,
      {
        appliedFilters,
    ...
    queryFn: () =>
      sdAdvertisingServicesAdGroupLevel.getSDProductAdsData(
        appliedFilters,
        advertisingFiltersWithNoDownload,
        paginationModel.pageIndex + 1,
        paginationModel.pageSize,
    ...
    enabled: selectedAdvertisingNavTitle === SdAdGroupLevelTitles.PRODUCT_ADS,
  });
  ...
  return (
    <AdvertisingRenderingComponents
      ...
      exportFileTitle={genExportFileName('amazon-sd', 'Product Ads')}
      ...
      areTableHeaderActionButtonsRequired={true}
    />
  );
}
