...
import { analysisAmzSDPerformanceOptions } from '@/constants/impact-analysis-filter.constants';
import { AmazonAdvertisingTableTypesEnum } from '@/enums/advertising.enums';
...
import { sdAdvertisingServices } from 'src/services/advertising/amazon/sd-advertising.service';
...
export default function AnalysisProductsPage() {
  ...
  useEffect(() => {
    ...
    const selectedTab = getSelectedNavTab(
      analysisAmzSDPerformanceOptions,
      ImpactAnalysisTableTitles.PRODUCT_ADS
    );
    dispatch(setSelectedAnalysisNavTab(selectedTab));
    dispatch(
      setSelectedAnalysisNavTitle(ImpactAnalysisTableTitles.PRODUCT_ADS)
    );
  }, [dispatch]);

  const fetchAnalysisGraphData = useAppQuery({
    queryFn: () => {
      return sdAdvertisingServices.getSDPerformanceGraph(
        appliedFilters,
    ...
    enabled: selectedAnalysisNavTitle === ImpactAnalysisTableTitles.PRODUCT_ADS,
  });
  ...
  const fetchAnalysisTableData = useAppQuery({
    queryFn: ({ signal }) => {
      return AnalysisSDService.getImpactAnalysisTable(
        appliedFilters,
        getAnalysisTableFilters(filters, MarketplaceEnum.AMAZON),
  ...
  return (
    <AnalysisRenderingComponents
      ...
      performanceNavigationTabOptions={analysisAmzSDPerformanceOptions}
