...
import { IEditAccessWalmartKeywordTargeting } from '@/interfaces/edit-access/edit-access.interface';
import {
  IAPIResponse,
  IErrorResultDetails,
...
export default function WalmartOverallAccountLevelKT() {
  ...
  const { mutateAsync: editAccessMutateData } = useAppMutation({
    mutationFn: (body: IEditAccessWalmartKeywordTargeting[]) =>
      walmartEditAccessOverallServices.updateWalmartOverallKeywordTargeting(
        body
      ),
    options: {
      onSettled: () => {
      ...
      onSuccess: (data) => {
        queryClient.invalidateQueries({
          queryKey: [QueryKeyEnums.WMT_OVERALL_ACCOUNT_LVL_KT_FETCH],
        });
  ...
  const { mutateAsync: editAccessMutateBidderData } = useAppMutation({
    mutationFn: (updatedBidders: IBidderUpdate[]) =>
      BidderServices.updateWalmartKeywordBidder(updatedBidders),
    options: {
      onSettled: () => {
        setIsEditLoading(false);
        setOpenSaveModal(false);
  ...
  const { mutateAsync: editAccessMutateAll } = useAppMutation({
    mutationFn: ({ body }: { body: IEditAccessWalmartKeywordTargeting[] }) => {
      ...
      onSettled: (responses) => {
        ...
        if (responses) {
          if (responses[0].status === 'fulfilled') {
            ...
            dispatch(
              showSuccessToastMessage({
                title: 'Keyword Targets updated successfully!',
              })
            );
          ...
          if (rejected && rejected?.length > 0) {
            ...
            if (rejected?.length !== responses?.length) {
              if (responses[0].status === 'rejected') {
                dispatch(
                  showErrorToastMessage({
                    title: 'Keyword Target data Update Failed',
                    description:
                      'There was an issue while updating Keyword Target data. Please try again.',
                  })
                );
              }
              ...
              dispatch(
                showErrorToastMessage({
                  title: 'Keyword Targets Update Failed',
                  description:
                    'There was an issue while updating the Keyword Targets. Please try again.',
                })
              );
            }
          }
  ...
  const confirmEditSaveClick = async () => {
    ...
    const editStateMap = getEditedTableValuesMap(
      updatedValues,
      editState
    ) as Map<string, IWalmartOverallKeywords>;
    ...
    if (updatedKeywordTargets.length) {
      ...
      if (isReview) {
        const popupParams: ICustomizablePopupDetails = {
          description: [
            {
              content: `<b>Please note: </b> Resuming a keyword in SB or SV campaign will trigger a review process by Walmart's team. This process may take 24-48 hours during which the campaign will stop serving ads.`,
              isHeading: false,
            },
            {
  ...
  const getOverallKeywordTargetingDownload = useCallback(
  ...
  const fetchKeywordTargeting = useAppQuery({
    queryKey: [
      QueryKeyEnums.WMT_OVERALL_ACCOUNT_LVL_KT_FETCH,
      {
        appliedFilters,
    ...
    queryFn: () =>
      walmartOverallAdvertisingServices.getOverallKeywords(
  ...
  const handleDownload = useCallback(
    async (isAllDownload: boolean) => {
      ...
      const data: Record<string, unknown>[] =
        (await getOverallKeywordTargetingDownload(
          true,
          isAllDownload
    ...
    [dispatch, getOverallKeywordTargetingDownload]
  );
