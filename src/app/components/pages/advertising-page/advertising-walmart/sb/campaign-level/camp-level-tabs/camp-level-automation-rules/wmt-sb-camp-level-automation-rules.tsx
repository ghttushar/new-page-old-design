...
import { WalmartSBCampaignLevelTitles } from '@/enums/advertising.enums';
...
} from '@/interfaces/advertising/walmart/walmart-sb-advertising.interface';
...
import { walmartSbAdvertisingServices } from '@/services/advertising/walmart/walmart-sb-advertising.service';
...
export default function AdvertisingWalmartSBCampLevelAutomationRules<
...
>({
  ...
  const { mutateAsync: editAccessMutate } = useAppMutation({
    mutationFn: async (body: IEditAccessAutomationRulesUpdateBody) => {
      return await walmartEditAccessSBServices.updateWalmartSBAutomationRules(
        body
      );
    },
    options: {
      ...
      onSuccess: (response) => {
        queryClient.invalidateQueries({
          queryKey: [QueryKeyEnums.WMT_SB_CAMPAIGN_LVL_AUTOMATION_RULES_FETCH],
        });
      ...
      onError(error, variables) {
        if (error && error.response) {
          ...
          if (
            ...
            queryClient.invalidateQueries({
              queryKey: [
                QueryKeyEnums.WMT_SB_CAMPAIGN_LVL_AUTOMATION_RULES_FETCH,
              ],
            });
  ...
  const confirmEditSaveClick = async () => {
    ...
    const editStateMap = getEditedTableValuesMap(
      updatedValues,
      editState
    ) as Map<string, IWalmartSBAutomationRules>;
  ...
  const fetchSBAutomationRules = useAppQuery({
    ...
    queryFn: () =>
      walmartSbAdvertisingServices.getSBCampaignAutomationRules(
  ...
  return (
    <AdvertisingRenderingComponents
      ...
      exportFileTitle={genExportFileName('walmart-sb', 'Campaign Rules')}
