...
export default function AdvertisingWalmartSBCampLevelProductAds<
...
>({
  ...
  const { mutateAsync: editAccessMutate } = useAppMutation({
    ...
    options: {
      ...
      onSuccess: (data, variables) => {
        ...
        queryClient.invalidateQueries({
          queryKey: [QueryKeyEnums.WMT_SB_CAMPAIGN_LVL_PRODUCT_ADS_FETCH],
        });
  ...
  useEffect(() => {
    ...
    dispatch(
      setSelectedAdvertisingNavTitle(WalmartSBCampaignLevelTitles.AD_ITEMS)
    );
  ...
  const fetchProductAds = useAppQuery({
    queryKey: [
      QueryKeyEnums.WMT_SB_CAMPAIGN_LVL_PRODUCT_ADS_FETCH,
      {
        appliedFilters,
    ...
    queryFn: () =>
      walmartSbAdvertisingServices.getSBAdItems(
        ...
        paginationModel.pageIndex + 1,
        paginationModel.pageSize,
        getFormattedSortModel(WalmartSBCampaignLevelTitles.AD_ITEMS, sortModel),
        searchText
      ),
    enabled:
      selectedAdvertisingNavTitle === WalmartSBCampaignLevelTitles.AD_ITEMS,
  });
