...
import { walmartSbCampaignLevelPerformanceOptions } from '@/constants/advertising-filter.constants';
...
import AdvertisingWalmartSBCampLevelSubWrapper from './wmt-sb-camp-level-sub-wrapper';

export default function AdvertisingWalmartSBCampLevel() {
  ...
  const fetchSelectedCampaign = useAppQuery({
    queryKey: [QueryKeyEnums.WMT_SB_CAMPAIGN_LVL_FETCH, campaignId],
  ...
  useEffect(() => {
    ...
    setUpdatedPerformanceOptions(
      getInitializedNavOptions(walmartSbCampaignLevelPerformanceOptions)
    );
    ...
    if (fetchSelectedCampaign.data) {
      ...
      setUpdatedPerformanceOptions(walmartSbCampaignLevelPerformanceOptions);
    }
  ...
  return (
    <AdvertisingWalmartSBCampLevelSubWrapper
