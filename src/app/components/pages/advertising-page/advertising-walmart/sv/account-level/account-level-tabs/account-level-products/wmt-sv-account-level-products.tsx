...
export default function AdvertisingWalmartSVAccountLevelProductAds() {
  ...
  useEffect(() => {
    ...
    dispatch(
      setSelectedAdvertisingNavTitle(WalmartSVAccountLevelTitles.AD_ITEMS)
    );
  }, [dispatch]);
    ...
    options: {
      ...
      onSuccess: (data) => {
        queryClient.invalidateQueries({
          queryKey: [QueryKeyEnums.WMT_SV_ACCOUNT_LVL_PRODUCT_ADS_FETCH],
        });
  ...
  const getSVAdItemsDownload = useCallback(
  ...
  const fetchProductAds = useAppQuery({
    queryKey: [
      QueryKeyEnums.WMT_SV_ACCOUNT_LVL_PRODUCT_ADS_FETCH,
      {
        appliedFilters,
    ...
    queryFn: () =>
      walmartSvAdvertisingServices.getSVAdItems(
        ...
        paginationModel.pageIndex + 1,
        paginationModel.pageSize,
        getFormattedSortModel(WalmartSVAccountLevelTitles.AD_ITEMS, sortModel),
        searchText
      ),
    enabled:
      selectedAdvertisingNavTitle === WalmartSVAccountLevelTitles.AD_ITEMS,
  });
  ...
  const handleDownload = useCallback(
    async (isAllDownload: boolean) => {
      ...
      const data: Record<string, unknown>[] = (await getSVAdItemsDownload(
        true,
        isAllDownload
