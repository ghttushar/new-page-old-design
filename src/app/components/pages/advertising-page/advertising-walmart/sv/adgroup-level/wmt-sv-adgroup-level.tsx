...
import { walmartSvAdGroupLevelPerformanceOptions } from 'src/constants/advertising-filter.constants';
import {
  IWalmartAdGroup,
  IWalmartCampaign,
...
import AdvertisingWalmartSVAdGroupLevelSubWrapper from './wmt-sv-adgroup-level-sub-wrapper';

export default function AdvertisingWalmartSVAdGroupLevel<
...
>({ selectedCampaign }: IAdvertisingAdGroupLevelProps<T>) {
  ...
  const fetchSelectedAdGroup = useAppQuery({
    queryKey: [QueryKeyEnums.WMT_SV_ADGROUP_LVL_FETCH, adGroupId],
  ...
  useEffect(() => {
    ...
    setUpdatedPerformanceOptions(
      getInitializedNavOptions(walmartSvAdGroupLevelPerformanceOptions)
    );
    ...
    if (fetchSelectedAdGroup.data) {
      ...
      if (Array.isArray(dataArray) && dataArray.length > 0) {
        ...
        setUpdatedPerformanceOptions(walmartSvAdGroupLevelPerformanceOptions);
      }
    }
  ...
  return (
    <AdvertisingWalmartSVAdGroupLevelSubWrapper
