...
export default function AdvertisingWalmartSVCampLevelProductAds<
...
>({
  ...
  useEffect(() => {
    ...
    dispatch(
      setSelectedAdvertisingNavTitle(WalmartSVCampaignLevelTitles.AD_ITEMS)
    );
  ...
  const { mutateAsync: editAccessMutate } = useAppMutation({
    ...
    options: {
      ...
      onSuccess: (data, variables) => {
        ...
        queryClient.invalidateQueries({
          queryKey: [QueryKeyEnums.WMT_SV_CAMPAIGN_LVL_PRODUCT_ADS_FETCH],
        });
  ...
  const getSVProductsDownload = useCallback(
  ...
  const fetchProductAds = useAppQuery({
    queryKey: [
      QueryKeyEnums.WMT_SV_CAMPAIGN_LVL_PRODUCT_ADS_FETCH,
      {
        appliedFilters,
    ...
    queryFn: () =>
      walmartSvAdvertisingServices.getSVAdItems(
        ...
        paginationModel.pageIndex + 1,
        paginationModel.pageSize,
        getFormattedSortModel(WalmartSVCampaignLevelTitles.AD_ITEMS, sortModel),
        searchText
      ),
    enabled:
      selectedAdvertisingNavTitle === WalmartSVCampaignLevelTitles.AD_ITEMS,
  });
  ...
  const handleDownload = useCallback(
    async (isAllDownload: boolean) => {
      ...
      const data: Record<string, unknown>[] = (await getSVProductsDownload(
        true,
        isAllDownload
    ...
    [dispatch, getSVProductsDownload]
  );
