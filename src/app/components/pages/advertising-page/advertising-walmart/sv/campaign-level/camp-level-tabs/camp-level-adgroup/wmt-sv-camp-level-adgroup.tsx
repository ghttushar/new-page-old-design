...
export default function AdvertisingWalmartSVCampLevelAdGroup<
...
>({
  ...
  const { mutateAsync: editAccessMutateAll } = useAppMutation({
    mutationFn: async ({
      ...
      return await walmartEditAccessSVServices.updateWalmartSVAdGroup(
        body.adGroups ?? []
      );
    },
    options: {
      ...
      onSuccess: (response, variables) => {
        ...
        queryClient.invalidateQueries({
          queryKey: [QueryKeyEnums.WMT_SV_CAMPAIGN_LVL_ADGROUPS_FETCH],
        });
  ...
  const confirmEditSaveClick = async () => {
    ...
    const confirmClick = async () =>
      await handleReviewPopupConfirmClick(isReview, updatedAdGroupsBody);

    if (isReview) {
      const popupParams: ICustomizablePopupDetails = {
        description: [
          {
            content: `<b>Please note: </b> Resuming an ad group in SV campaign will trigger a review process by Walmart's team. This process may take 24-48 hours during which the campaign will stop serving ads.`,
            isHeading: false,
          },
          {
  ...
  useEffect(() => {
    ...
    dispatch(
      setSelectedAdvertisingNavTitle(WalmartSVCampaignLevelTitles.AD_GROUPS)
    );
  }, [dispatch, updatedPerformanceOptions]);

  const getSVAdGroupDownload = useCallback(
  ...
  const fetchAdGroups = useAppQuery({
    queryKey: [
      QueryKeyEnums.WMT_SV_CAMPAIGN_LVL_ADGROUPS_FETCH,
      {
        appliedFilters,
    ...
    queryFn: () =>
      walmartSvAdvertisingServices.getSVAdGroups(
    ...
    enabled:
      selectedAdvertisingNavTitle === WalmartSVCampaignLevelTitles.AD_GROUPS,
  });
  ...
  const handleDownload = useCallback(
    async (isAllDownload: boolean) => {
      ...
      const data: Record<string, unknown>[] = (await getSVAdGroupDownload(
        true,
        isAllDownload
  ...
  return (
    <React.Fragment>
      <AdvertisingRenderingComponents
        ...
        exportFileTitle={genExportFileName('walmart-sv', 'Ad Groups')}
