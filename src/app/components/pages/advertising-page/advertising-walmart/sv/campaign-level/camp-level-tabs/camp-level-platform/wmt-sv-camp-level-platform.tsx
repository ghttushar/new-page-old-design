...
export default function AdvertisingWalmartSVCampLevelPlatform<
...
>({
  ...
  const setSelectedColumnsHandler = (
    ...
    columnFilterUtils.syncStoredColumnFilters(
      WalmartSVCampaignLevelTitles.PLATFORM,
      selectedColumns
    );
  ...
  useEffect(() => {
    ...
    dispatch(
      setSelectedAdvertisingNavTitle(WalmartSVCampaignLevelTitles.PLATFORM)
    );
  }, [dispatch, updatedPerformanceOptions]);

  const getSVPlatformDownload = useCallback(
  ...
  const fetchPlatform = useAppQuery({
    queryKey: [
      QueryKeyEnums.WMT_SV_CAMPAIGN_LVL_PLATFORM_FETCH,
      {
        appliedFilters,
    ...
    queryFn: () =>
      walmartSvAdvertisingServices.getSVPlatform(
        getWalmartAppliedFilters(appliedFilters, false, false),
        advertisingFiltersWithNoDownload,
        paginationModel.pageIndex + 1,
        paginationModel.pageSize,
        getFormattedSortModel(WalmartSVCampaignLevelTitles.PLATFORM, sortModel),
        searchText
      ),
    enabled:
      selectedAdvertisingNavTitle === WalmartSVCampaignLevelTitles.PLATFORM,
  });
  ...
  const handleDownload = useCallback(
    async (isAllDownload: boolean) => {
      ...
      const data: Record<string, unknown>[] = (await getSVPlatformDownload(
        true,
        isAllDownload
    ...
    [dispatch, getSVPlatformDownload]
  );
  ...
  return (
    <AdvertisingRenderingComponents
      ...
      exportFileTitle={genExportFileName('walmart-sv', 'Platform')}
