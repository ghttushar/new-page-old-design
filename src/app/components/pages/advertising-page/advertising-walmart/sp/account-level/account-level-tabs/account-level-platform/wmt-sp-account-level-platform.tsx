...
export default function AdvertisingWalmartSPAccountLevelPlatform() {
  ...
  const setSelectedColumnsHandler = (
    ...
    columnFilterUtils.syncStoredColumnFilters(
      WalmartSPAccountLevelTitles.PLATFORM,
      selectedColumns
    );
  ...
  const { mutateAsync: editAccessMutate } = useAppMutation({
    mutationFn: (body: IEditAccessWalmartPlatform[]) =>
      walmartEditAccessSPServices.updateWalmartSPPlatform(body),
    options: {
      onSettled: () => {
      ...
      onSuccess: (data) => {
        queryClient.invalidateQueries({
          queryKey: [QueryKeyEnums.WMT_SP_ACCOUNT_LVL_PLATFORM_FETCH],
        });
  ...
  useEffect(() => {
    ...
    dispatch(
      setSelectedAdvertisingNavTitle(WalmartSPAccountLevelTitles.PLATFORM)
    );
  }, [dispatch]);

  const getPlatformDownload = useCallback(
  ...
  const fetchPlatform = useAppQuery({
    queryKey: [
      QueryKeyEnums.WMT_SP_ACCOUNT_LVL_PLATFORM_FETCH,
      {
        appliedFilters,
    ...
    queryFn: () =>
      walmartSpAdvertisingServices.getPlatform(
        ...
        paginationModel.pageIndex + 1,
        paginationModel.pageSize,
        getFormattedSortModel(WalmartSPAccountLevelTitles.PLATFORM, sortModel),
        searchText
      ),
    enabled:
      selectedAdvertisingNavTitle === WalmartSPAccountLevelTitles.PLATFORM,
  });
  ...
  const handleDownload = useCallback(
    async (isAllDownload: boolean) => {
      ...
      const data: Record<string, unknown>[] = (await getPlatformDownload(
        true,
        isAllDownload
  ...
  return (
    <AdvertisingRenderingComponents
      ...
      exportFileTitle={genExportFileName('walmart-sp', 'Platform')}
